import copy,random,os,csv,wandb
...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        if random.random() < 0.01:
            self.writereward(self.csv_path, rewards,mask, L_td,L_td,L_td, t_env)
    ...
    def writereward(self, path,reward, mask, td_loss, loss,id_consist_loss,step):
        reward = reward.sum()/mask.sum()
        if self.args.wandb:
            # wandb.log({f"{phase} MSE loss": epoch_mse_loss})
            wandb.log({'step': step, " TD_Loss": td_loss, "Training reward": reward})
        if os.path.isfile(path):
            with open(path, 'a+') as f:
                csv_write = csv.writer(f)
                csv_write.writerow(
                    [step, reward.item(),reward.item(), td_loss.item(), loss.item()])
        else:
            with open(path, 'w') as f:
                csv_write = csv.writer(f)
                csv_write.writerow(
                    ['step', 'reward', 'TD_Loss', 'Final Loss'])
                csv_write.writerow(
                    [step, reward.item(),td_loss.item(), loss.item()])
